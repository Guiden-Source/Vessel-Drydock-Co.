...
Cada item (tabela `reading_items`):
...
- `progress` → opcional, número entre 0 e 1 (por exemplo 0.75).
...
- Deixar ações antigas como se fossem atuais por falta de filtro de snapshot.
...
- Tentar compartilhar backlog entre empresas sem isolamento.
- Não deixar claro quais itens migraram ou foram reavaliados em snapshots seguintes.
...
Tabela genérica `key_indicators` ligada ao snapshot.
...
- `snapshot_id`.
- `organization_id`.
- `title` → ex.: "Tráfego do Site", "Social", "ROI Marketing", "Saúde Digital".
...
Para o MVP, você pode registrar sempre os mesmos 4 títulos e tipos.

### 7.2. Possíveis erros

- Tratar tudo como string e perder semântica (difícil gerar gráficos/alertas depois).
- Misturar períodos (tráfego semanal com ROI mensal) sem indicar isso em `period_start`/`period_end`.
...
- Tabelas sem `organization_id` e relying apenas em `snapshot_id`.
...
Um payload mínimo que o front precisa para renderizar o dashboard pode ser:
...
6. **Usar esse modelo como base** para o schema SQL do Supabase e para o formato de entrada (planilha/JSON) do processo diário de publicação.
```
